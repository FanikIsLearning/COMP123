...
namespace Atom
{
    ...
    {
        static List<Atom> elements = Atom.GetAtoms();
        static void Main(string[] args)
        {
            ...
            writeFirstItemXml(elements, "first-element.xml");

            // 3 - Read the above file and display the item
            readFirstItemXml("first-element.xml");

            // 6. Serialize the first item using JSON format and save to a file
            writeFirstItemJson(elements, "hydrogen.json");

            // 7. Read the file and display the item
            readFirstItemJson("hydrogen.json");

            // 8. Serialize the entire collection using JSON format and save to a file
            writePeriodicTable(elements, "elements.json");

            // 9. Read the file and display all the items
            readPeriodicTable("elements.json");
        }

        static void writeFirstItemXml(List<Atom> atoms, string filename)
        {
            XmlSerializer serializer = new XmlSerializer(typeof(Atom));
            TextWriter xmlWriter = new StreamWriter(filename);
            serializer.Serialize(xmlWriter, elements[0]);
        }

        static void readFirstItemXml(string filename)
        {
            XmlSerializer serializer = new XmlSerializer(typeof(Atom));
            TextReader xmlReader = new StreamReader(filename);
            Atom atom = (Atom)serializer.Deserialize(xmlReader);
            Console.WriteLine("\nFirst Element from XML file:");
            Console.WriteLine("----------------------------------------------------------------------------------");
            Console.WriteLine(atom);
            Console.WriteLine("==================================================================================");
        }

        static void writeFirstItemJson(List<Atom> atoms, string filename)
        {
            Atom hydrogen = elements[0];
            string jsonWriter = JsonSerializer.Serialize(hydrogen);
            File.WriteAllText(filename, jsonWriter);
        }

        static void readFirstItemJson(string filename)
        {
            TextReader jsonReader = new StreamReader(filename);
            Atom hydrogen = JsonSerializer.Deserialize<Atom> (jsonReader.ReadToEnd());
            Console.WriteLine("\nFirst Element from JSON file:");
            Console.WriteLine("----------------------------------------------------------------------------------");
            Console.WriteLine(hydrogen);
            Console.WriteLine("==================================================================================");
        }

        static void writePeriodicTable(List<Atom> atoms, string filename)
        {
            string jsonWriter = JsonSerializer.Serialize(elements);
            File.WriteAllText(filename, jsonWriter);
        }

        static void readPeriodicTable(string filename)
        {
            TextReader jsonReader = new StreamReader(filename);
            List<Atom> elements = JsonSerializer.Deserialize<List<Atom>>(jsonReader.ReadToEnd());
            Console.WriteLine("\nPeriodic Table:");
            Console.WriteLine("----------------------------------------------------------------------------------");
